...
# original path was /Users/ndai/Desktop/RGD/.env (machine-specific, doesn't exist here) --
# repointed to this project's established .env (same DB host used throughout this session).
...
engine = create_engine(
    url,
    pool_pre_ping=True,
    # read_timeout was 900s (15min) originally -- COHORT_SQL's GROUP BY/GROUP_CONCAT
    # aggregation over the full diagnosis table blew past that and killed the socket
    # mid-query ("Lost connection to MySQL server during query (timed out)"); bumped
    # to 2 hours since this DB is measurably slow on unindexed full-table aggregations.
    connect_args={"connect_timeout": 30, "read_timeout": 7200, "write_timeout": 7200},
)

# ── Checkpoint + retry helper ─────────────────────────────────────────────────────────
# Added after TWO separate multi-hour runs each died to a mid-query dropped connection
# at a DIFFERENT step (first: client-side read_timeout at 900s; second: a plain "Lost
# connection to MySQL server during query" on a much cheaper query, so not a timeout --
# something in the network path or the DB server itself is killing long-idle sockets).
# Rather than guess at a single root cause, every heavy query now (a) retries with a
# fresh connection on any drop, and (b) checkpoints its result to disk so a restart
# after a late-stage failure doesn't have to re-run the expensive early steps again.
import pickle
...
print(f"Qualifying MS diagnosis rows: {len(ms_diagnoses):,}")
...
subtype_first_seen AS (
    -- one row per (ndid, ms_subtype): the date that subtype was first recorded for the patient
...
SELECT
    ...
    -- one flag + one date column per subtype category, so each is directly filterable/joinable
...
print(f"MS cohort size (distinct patients): {len(ms_cohort):,}")
ms_cohort.head()
...
SELECT
    ...
    CASE
        ...
        ELSE 'Unknown'
    END                                                      AS age_group,
    -- clinical onset category (mutually exclusive, distinct from the decade age_group above):
    -- pediatric-onset uses the literature's <18 cutoff, not the <20 decade bin
    CASE
...
FROM (
    SELECT
        cohort.*,
        p.dob                                                        AS birth_year,
        -- CAST to SIGNED first: YEAR() is UNSIGNED, so dob > diagnosis_year (bad birth-year
        -- data) would otherwise underflow into "BIGINT UNSIGNED out of range" (MySQL #1690)
        -- instead of a sensible negative number.
        -- only exclude a truly bad value (missing dob, or dob after the diagnosis year);
        -- any other non-negative age, however large, is kept as a real age (not capped)
        CASE
...
# ===== cell 7 =====
# (generic_name, drug_class, efficacy_tier, route, [brand/generic search terms], [exclude terms])
# exclude terms: a positive term match is DISCARDED if med_name/std_BN/std_LN also contains
# one of these -- needed because "Peginterferon beta-1a" contains "interferon beta-1a" as a
# literal substring, so without this "Interferon beta-1a" (listed first) would wrongly
# absorb Plegridy/peginterferon records via LIKE '%interferon beta-1a%'.
DMT_REFERENCE = [
...
# ===== cell 8 =====
# ── Build the three parallel CASE/WHEN fragments (generic name, class, efficacy tier) ──
...
# ===== cell 9 =====
# ── Run in batches over the cohort's ndids (ndid filter FIRST, via FORCE INDEX) ──────
# med_start_date fallback chain: med_start_date -> med_fill_date -> enc_date. Built as
# COALESCE(...) AS med_start_date directly
#
# Per-batch checkpointing added (this loop is the single longest-running piece, ~25-40
# min): each batch's result is saved to disk as soon as it's fetched, and a restart
# skips every batch that's already on disk instead of re-querying from batch 0. Each
# batch also gets its own retry-with-fresh-connection loop, same rationale as
# checkpointed_read_sql above.
BATCH_DIR = os.path.join(CHECKPOINT_DIR, "ms_dmt_batches")
os.makedirs(BATCH_DIR, exist_ok=True)
FINAL_MS_DMT_PATH = os.path.join(CHECKPOINT_DIR, "ms_dmt_final.pkl")

BATCH_SIZE = 1000
cohort_ndids = ms_cohort_demo["ndid"].astype(str).tolist()

if os.path.exists(FINAL_MS_DMT_PATH):
    ms_dmt = pd.read_pickle(FINAL_MS_DMT_PATH)
    print(f"[checkpoint] ms_dmt: loaded {len(ms_dmt):,} rows from {FINAL_MS_DMT_PATH}", flush=True)
else:
    t_start = time.time()
    ...
    for b in range(n_batches):
        ...
        for attempt in range(1, 5):
            try:
                ...
                print(f"[{done}/{len(cohort_ndids)}] batch {b} matches: {len(batch_df)}  "
...
print(f"\nTotal DMT medication rows: {len(ms_dmt):,}")
print(f"Distinct patients on a DMT: {ms_dmt['ndid'].nunique():,}")
print("\nmed_start_date source breakdown:")
print(ms_dmt["med_start_date_source"].value_counts(dropna=False))
ms_dmt.to_pickle(FINAL_MS_DMT_PATH)
ms_dmt.head()

# ===== cell 10 =====
# ── Overall DMT utilization: % of the MS cohort ever on >=1 DMT ──────────────────────
...
# ===== cell 11 =====
# ── Utilization by drug class: unique patients ever on that class / total cohort ────
# (not mutually exclusive -- a patient can appear under more than one class)
...
# ===== cell 12 =====
# ── Utilization by individual DMT, among DMT-treated patients only ──────────────────
# patients receiving each drug / number of DMT-treated patients (not the full cohort),
# ranked highest to lowest -- "most commonly used therapies"
...
# ===== cell 13 =====
# ── DMT utilization by subtype: N, % receiving each DMT class, within each subtype ───
...
# ===== cell 14 =====
# ── shared helper: drug_class x category matrix, N/% within each category's own N ───
...
# ===== cell 15 =====
# ── DMT utilization by onset category ────────────────────────────────────────────────
...
# ===== cell 16 =====
# ── Expand each DMT record into one row per (ndid, drug, class, active_year) ─────────
...
# missing end date -> still active through the study cutoff; cap any end date beyond cutoff too
...
]  # drop missing-start-date rows and any bad ranges
...
dmt_year_expanded = (
    ...
    .drop_duplicates()  # one row per (ndid, drug, class, year) even if multiple courses overlap same year
)
...
# cumulative MS patients diagnosed by each year (denominator for coverage)
...
# ===== cell 17 =====
# ── A. Absolute number: unique patients receiving each DMT/class, per year ──────────
...
# ===== cell 19 =====
# ── B. Annual coverage: (A) / MS patients diagnosed by that year, as a % ─────────────
...
# ===== cell 21 =====
# ── A. Absolute number: unique patients on each efficacy tier, per year ─────────────
...
# ===== cell 23 =====
# ── time_to_first_dmt: one row per DMT-treated patient ───────────────────────────────
...
# ===== cell 24 =====
# ── shared stats helper + subtype/age-group/onset breakdowns ────────────────────────
...
# by subtype (not mutually exclusive -- reuses the same "ever had this flag" definition)
...
# ===== cell 25 =====
# ── distinct DMT count per patient, reindexed to the full cohort (0 for untreated) ───
n_distinct_dmts = (
...
# ===== cell 26 =====
# ── among DMT-treated patients only: % single vs. multiple distinct DMTs ────────────
...
# ===== cell 27 =====
# ── ordered distinct-drug sequence per patient (by first exposure date) ──────────────
...
# ===== cell 28 =====
# ── explode each sequence into its individual step-by-step transitions ──────────────
transition_rows = []
...
# ===== cell 29 =====
# ── rank (from_drug -> to_drug) pairs by frequency, across all steps/patients ────────
...
# ===== cell 30 =====
# ── collapse to one row per (patient, drug) course ───────────────────────────────────
...
# ===== cell 31 =====
# ── discontinuation reasons, ranked (among discontinued courses only) ────────────────
...
# ===== cell 32 =====
# ── class of each patient's very first DMT record ────────────────────────────────────
...
# ===== cell 33 =====
# ── class of each patient's first DMT record, split by MS subtype ───────────────────
...
# ── icd_reference sheet ──────────────────────────────────────────────────────────────
# Original source was an external TSV (/Users/ndai/Desktop/RGD/.../ms_icd_codes.tsv) not
# present/reachable on this machine. Reconstructed directly from MS_ICD_REFERENCE_SQL
# (cell 2) instead of guessing at a replacement file -- this is actually the more
# trustworthy source anyway, since it's the literal table the cohort query itself joins
# against, guaranteed consistent with COHORT_SQL rather than a possibly-stale export.
...
# ── gender bucket: collapse whatever gender_hl7_std holds into Female/Male/Unknown ──
...
def _pct_rows(series, decimals=1):
    """Value counts (dropna=False, NaN -> 'Unknown') as (label, N, pct) rows, sorted desc."""
...
iqr = q3 - q1
# standard Tukey IQR rule: outside [Q1-1.5*IQR, Q3+1.5*IQR] is a statistical outlier
outlier_lo, outlier_hi = q1 - 1.5 * iqr, q3 + 1.5 * iqr
n_outliers = int(((age < outlier_lo) | (age > outlier_hi)).sum())

# rows: (Characteristic, N, %, Value) -- N/% populated for counts, Value populated for
# plain stats (mean/SD/min/max/etc, which aren't counts of anything)
rows = [
...
# % of patients who have ever had more than one distinct subtype recorded -- a proxy for
# subtype progression/transition (e.g. RRMS -> SPMS) captured purely from cohort data
...
# ── additional characteristics available from the columns we have ──────────────────
...
# "<<STATE>>" is a literal masking token in the source data (small-cell suppression for
# privacy), not a real state -- relabel it so it doesn't read as one in the report.
state_for_chars = (
...
# zip has much higher cardinality than state, so same top-10 + "Other" pattern
...
# ===== cell 36 =====
# ── subtype_characteristics: RRMS / PPMS / SPMS columns, computed on the "ever had this
# subtype" flag subsets (so a patient who has both RRMS and SPMS flags set appears in both
# columns) ───────────────────────────────────────────────────────────────────────────
...
# ===== cell 37 =====
# ── region cross-tabs (region = state) ───────────────────────────────────────────────
# "<<STATE>>" is a literal masking token in the source data (small-cell suppression for
# privacy), not a real state -- relabel it so it doesn't read as one in the report.
state_clean = (
    ms_cohort_demo["state"]
...
# sex_by_region: separate N and % column per gender bucket, per state
...
# subtype_by_region: separate N and % column per subtype, per state (not mutually exclusive)
...
# age_by_region: mean/median age at diagnosis per state (no % here, unaffected)
...
# ===== cell 38 =====
# ── data_quality: birth-year issue breakdown from dob_quality (built earlier) ────────
dq_counts = dob_quality["dob_quality_flag"].value_counts()
dq_order = ["plausible", "missing_dob", "negative_age", "implausible_over_100"]
# NOTE: none of these categories are excluded from the cohort -- missing_dob/negative_age
# patients get age_at_diagnosis = NULL ("Unknown" in every breakdown), and "over_100" patients
# keep their actual computed age (not treated as implausible/capped). This table is purely
# informational QC visibility into birth-year data quality, not an exclusion criterion.
dq_labels = {
...
# ===== cell 39 =====
# ── diagnoses_by_year: new-patient count by first_diagnosis_year, plus subtype mix (N and %
# in separate columns) and age trend (so both diagnosis volume AND the age/subtype profile
# of new patients can be tracked over time) ─────────────────────────────────────────────
...
# ===== cell 41 =====
# ── subtype_transitions: patient-level detail for those with >1 subtype ever recorded ──
...
# time-to-progression: years between the first and last recorded subtype -- how long it
# took for the documented subtype to change (e.g. RRMS -> SPMS)
...
# ===== cell 42 =====
# ── subtype_transition_patterns: aggregate counts + median time-to-progression per pattern ──
...
readme_rows = [
    ...
    ("icd_reference", SHEET_MARK, "The ICD-9/ICD-10 reference codes used to define the MS cohort (ms_icd_codes.tsv, verbatim)"),
    ("icd_reference", "icd_code", "ICD code as given in ms_icd_codes.tsv"),
    ...
    ("cohort_characteristics", SHEET_MARK,
     ...
     "n=the full MS cohort (no patients are excluded -- see the 'No patients excluded' methodology note below)"),
    ...
    ("cohort_characteristics", "Value (N (%))", "Either a plain number/stat (age summary rows), or 'N (xx.x%)' for every "
                                                  ...
                                                  "unless the row is itself a sub-breakdown (e.g. Female/Male are % of the full cohort, not of each other)"),
    ...
    ("subtype_characteristics", "Mean age at diagnosis / Median age", "age_at_diagnosis (ms_cohort_demo) within that subtype's subset"),
    ("subtype_characteristics", "Female / Male", "Count (%) within that subtype's subset, e.g. RRMS-Female% = Female RRMS patients / all RRMS patients"),
    ...
    ("age_by_region", "Mean age at diagnosis / Median age at diagnosis", "age_at_diagnosis (ms_cohort_demo), within that state"),
    ...
    ("diagnoses_by_year", "(index)", "first_diagnosis_year: the year of each patient's EARLIEST qualifying MS diagnosis"),
    ...
    ("diagnoses_by_year", "RRMS / PPMS / SPMS / Unspecified", "Count (%), among patients first diagnosed that year, who EVER (any year, "
                                                                "not just that year) had that subtype flag = 1 -- this is why Unspecified is "
                                                                "close to 100% in most years: see 'Subtype flags/percentages' methodology note below"),
    ...
    ("subtype_transitions", "subtype_sequence", "ms_subtypes_ever verbatim, e.g. 'MS_unspecified,RRMS,SPMS' -- ordered by when each was first seen"),
    ...
    ("subtype_transitions", "transition_pattern", "subtype_sequence joined with ' → ', e.g. 'RRMS → SPMS'"),
    ...
    ("subtype_transition_patterns", "% of multi-subtype patients", "N_patients / total patients with >1 subtype (the subtype_transitions row count)"),
    ...
    ("dmt_overall_utilization", SHEET_MARK, "% of the full MS cohort ever receiving >=1 DMT (post-diagnosis only)"),
    ("dmt_utilization_by_class", SHEET_MARK, "Unique patients ever on each drug class / the full cohort (not mutually exclusive)"),
    ("dmt_utilization_by_drug", SHEET_MARK, "Unique patients ever on each individual DMT / DMT-treated patients, most to least common"),
    ("dmt_utilization_by_subtype", SHEET_MARK, "drug_class x MS subtype matrix, N/% within each subtype's own patient count"),
    ("dmt_utilization_by_age_grp", SHEET_MARK, "drug_class x age_group matrix, N/% within each age group's own patient count"),
    ("dmt_utilization_by_onset", SHEET_MARK, "drug_class x onset_category matrix, N/% within each onset category's own patient count"),
    ("dmt_by_year_absolute_class", SHEET_MARK, "Unique patients per drug class ACTIVE in each calendar year (period prevalence, not just new starts)"),
    ...
    ("time_to_first_dmt", SHEET_MARK, "Median/Q1/Q3/IQR/Mean days from first_diagnosis_date to first DMT, and % treated within 30/90/365 days -- among DMT-treated patients only"),
    ("dmt_count_categories", SHEET_MARK, "0/1/2/3/4+ distinct DMTs ever received per patient, N/% of the full MS cohort (0 = never treated)"),
    ("dmt_single_vs_multiple", SHEET_MARK, "Among DMT-treated patients only: % receiving exactly 1 distinct DMT vs. >1 (\"multiple\") ever"),
    ("dmt_sequences", SHEET_MARK, "One row per multi-DMT patient: full ordered drug_sequence (by first-exposure date) and n_drugs"),
    ("dmt_transitions_long", SHEET_MARK, "One row per individual switch step (step=1 is 1st->2nd drug, step=2 is 2nd->3rd, ...) within each multi-DMT patient's sequence"),
    ("dmt_transition_ranking", SHEET_MARK, "(from_drug -> to_drug) pairs ranked most-to-least common across all steps/patients, with % of transitions and % of multi-DMT patients"),
    ("first_dmt_class_distribution", SHEET_MARK, "drug_class of each DMT-treated patient's very FIRST medication record (not \"ever received\"), ranked by % of treated patients"),
    ("dmt_by_year_abs_efficacy", SHEET_MARK, "Unique patients per efficacy tier (High vs Standard) ACTIVE in each calendar year -- answers \"has treatment shifted toward high-efficacy therapies\""),
    ("dmt_by_year_cov_efficacy", SHEET_MARK, "dmt_by_year_abs_efficacy / cumulative patients diagnosed by that year, as a %"),
    ("time_to_dmt_by_subtype", SHEET_MARK, "Median/Q1/Q3/IQR/Mean days to first DMT + % within 30/90/365 days, split by MS subtype (not mutually exclusive)"),
    ...
    ("first_dmt_class_by_subtype", SHEET_MARK, "drug_class of each patient's FIRST DMT record, split by MS subtype (not mutually exclusive)"),
    ("dmt_courses", SHEET_MARK, "One row per (patient, drug) course: course_start/course_end/discont_date/discont_reason/discontinued flag/persistence_days -- the detail table behind discontinuation_summary and discontinuation_reasons"),
    ("discontinuation_summary", SHEET_MARK, "N courses, % discontinued (discont_date present), and persistence (days from course start to discont_date: Median/Q1/Q3/IQR/Mean) among discontinued courses only"),
    ("discontinuation_reasons", SHEET_MARK, "discont_reason ranked most-to-least common, among discontinued courses only"),
    ("Methodology", "Cohort definition", "≥1 qualifying MS diagnosis (ICD-10 G35 family or legacy ICD-9 340), "
                                          "udm_active_flag = 'Y' only. No date cutoff -- all available data is used"),
    ("Methodology", "No patients excluded", "Every patient in the cohort appears in every sheet. Patients with a "
                                              ...
                                              "value, not capped or treated as an error. See data_quality for the "
                                              "informational-only QC breakdown of these birth-year issues"),
    ("Methodology", "\"Region\"", "= patients.pat_state (only geographic field at usable granularity)"),
    ("Methodology", "Subtype flags/percentages", "NOT mutually exclusive -- mean \"ever diagnosed with X\"; a patient who progressed "
                                                    "RRMS->SPMS counts toward both. This is also why diagnoses_by_year's Unspecified column "
                                                    "is close to 100% in most years: providers overwhelmingly also use the plain G35/G35.D "
                                                    "'unspecified' code even when a more specific subtype code is also on file"),
    ...
    ("Methodology", "DMT / treatment data", "NOT included in this workbook yet -- cohort/diagnosis/demographics only"),
]
...
# The request asked for "separate tabs ... with a heading explaining the analysis".
# The notebook already builds a very thorough README sheet (per-column descriptions);
# this reuses those same SHEET_MARK description strings as a one-line merged banner
# directly on top of EACH sheet too, so the explanation is visible without needing to
# flip back to README every time.
...
wb.save(OUT_PATH)
print(f"Added heading banners to {len(wb.sheetnames) - 2} tabs; re-saved {OUT_PATH}")
